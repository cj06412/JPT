...
## 8. 行走五帧（5-frame walk cycle）—— GPT-Image 提示词

> 行为模型：站 10 秒 → 走 5 秒 循环。站立 2 帧呼吸（`jpt-stand1.png` /
> `jpt-stand2.png`，已做）。走路 **5 帧独立图片**，按**走过的距离**切帧
> （每 14px 一帧，不是按时间）：
>
> | 文件 | 帧（建议） | 状态 |
> |---|---|---|
> | `walk1.png` | 接触 A（左腿前迈步） | ✅ 已有真图 |
> | `walk2.png` | 过渡 A（并脚收脚） | ✅ 已有真图 |
> | `walk3.png` | 腾空过渡（摆腿、身体抬高） | ⬜ 待补 |
> | `walk4.png` | 接触 B（右腿前，walk1 的镜像） | ⬜ 待补 |
> | `walk5.png` | 过渡 B（另一侧并脚收脚） | ⬜ 待补 |
>
> 循环：1→2→3→4→5→1。代码已接好（walk3/4/5 是占位红块），换真图即
> 生效，无需改代码。「帧（建议）」只是参考，你 5 张怎么编排都行——代码就
> 按顺序循环；上下颠目前打在第 2、4 帧（`WALK_BOB_FRAMES`，一行可调）。

### 8.1 关键一致性要求（最重要，决定切帧会不会“抖”）

GPT-Image 用 **image-to-image**：补 walk3/walk4/walk5 时把 **`walk1.png`
一起喂进去**当人物+配色+构图+缩放基准。新帧**只有腿和手臂变**，其余一律不变：

- 同一个人：大蓬松深红/酒红卷发、戴眼镜、浅肤
- 同一套衣服：敞口深红外套 + 深色格纹内搭、蓝牛仔裤、深色鞋浅色鞋底
- **同机位、同缩放、同居中位置、同光照**——和 `walk1.png` 完全一致（关键：否则循环时角色跳动）
...
### 8.2 Base prompt（walk3 / walk4 / walk5 公用，粘在前面）
...
### 8.3 待补三帧的姿势（把对应一句接在 Base prompt 后面）

| 文件 | 帧 | 接在 base 后面的英文 |
|---|---|---|
| `walk3.png` | 腾空过渡 | `Walk-cycle PASSING/LIFT frame: legs passing under the torso with the rear leg swinging through mid-air toward the next step, the body at the HIGHEST point of the stride (clearly lifted up), one supporting leg fairly straight; arms mid-swing near neutral. An in-between airborne pose, clearly different from a flat standing idle.` |
| `walk4.png` | 接触 B（镜像迈步） | `Walk-cycle CONTACT frame, mirrored stride: the RIGHT leg is striding forward with a bent knee and the heel landing ahead, the LEFT leg is extended trailing behind pushing off, slight forward body lean; LEFT arm swung forward, RIGHT arm swung back. Same strong stride length as the attached reference but with the opposite leg and arm forward.` |
| `walk5.png` | 过渡 B（另一侧并脚） | `Walk-cycle PASSING frame, other side: both legs close together passing directly under the torso, the RIGHT foot is the planted support leg and the LEFT leg is swinging through about to step, body slightly settled (low point), arms near neutral close to the sides. Calm in-between pose, distinct from a standing idle.` |
...
### 8.5 验收（walk3 / walk4 / walk5 三张要过）

- [ ] 和 `walk1.png` 一眼同一个人、同配色、同机位同缩放
- [ ] 透明背景干净（无地面/无投影/无白边）
- [ ] walk3 明显抬高（腾空感）；walk4 是 walk1 的镜像迈步（另一条腿在前）；walk5 另一侧并脚、不像呆站
- [ ] 1→2→3→4→5 连起来是顺滑走路循环（左右腿交替、有上下起伏）
- [ ] 命名严格 `walk3.png` / `walk4.png` / `walk5.png`，覆盖进 `assets/sprites/`
