...
> | `jpt-walk2.png` | 过渡 A（并脚收脚） | ✅ 已有真图 |
> | `jpt-walk3.png` | 腾空过渡（摆腿、身体抬高） | ⬜ 待补 |
> | `jpt-walk4.png` | 接触 B（右腿前，walk1 的镜像） | ⬜ 待补 |
> | `jpt-walk5.png` | 过渡 B（另一侧并脚收脚） | ⬜ 待补 |
>
...
GPT-Image 用 **image-to-image**：补 walk3/walk4/walk5 时把 **`jpt-walk1.png`
...
| `jpt-walk4.png` | 接触 B（镜像迈步） | `Walk-cycle CONTACT frame, mirrored stride: the RIGHT leg is striding forward with a bent knee and the heel landing ahead, the LEFT leg is extended trailing behind pushing off, slight forward body lean; LEFT arm swung forward, RIGHT arm swung back. Same strong stride length as the attached reference but with the opposite leg and arm forward.` |
| `jpt-walk5.png` | 过渡 B（另一侧并脚） | `Walk-cycle PASSING frame, other side: both legs close together passing directly under the torso, the RIGHT foot is the planted support leg and the LEFT leg is swinging through about to step, body slightly settled (low point), arms near neutral close to the sides. Calm in-between pose, distinct from a standing idle.` |
...
- [ ] 命名严格 `jpt-walk3.png` / `jpt-walk4.png` / `jpt-walk5.png`，覆盖进 `assets/sprites/`
