...
## 8. v1.5 行走四帧（4-frame walk cycle）—— GPT-Image 提示词

> 行为模型已改：站立 10 秒 → 走 5 秒 循环。站立用 2 帧呼吸（你已做的
> `jpt-stand1.png` / `jpt-stand2.png`）。走路用 **4 帧独立图片**：
> `jpt-walk1.png` `jpt-walk2.png` `jpt-walk3.png` `jpt-walk4.png`。
> 代码已接好，换这 4 个文件即生效，无需改代码。

### 8.1 关键一致性要求（最重要，决定四帧切换会不会“抖”）

GPT-Image 用 **image-to-image**：每帧都把 **`jpt-stand1.png`（或你附的那张走路参考图）一起喂进去**当身份+构图基准。四帧之间**只有腿和手臂在动**，其它一律不变：

- 同一个人：同脸、同发型（深蓝黑短发微乱）、同配色
- 同一套衣服：红色短袖、深炭灰短裤、棕色靴子
- **同机位、同缩放、同居中位置、同光照**——和 `jpt-stand1/2` 完全一致（关键：否则循环时角色会跳动）
- 透明背景（PNG alpha 干净，无地面、无阴影投到背景、无白边）
- SDV 画风：扁平 cel-shading 色块、1px 深色外描边、无照片感 / 无渐变 / 无模糊 / 无景深
- 正面略 3/4 视角全身（和你附的参考图一致），人物完整在画面内、底部留一点边距
- 输出大尺寸方图（1024×1024 或更大，和 stand 同比例），后续不用 Aseprite 也能直接用

### 8.2 Base prompt（四帧公用，粘在每个 prompt 前面）

```
Stardew Valley style full-body game character, the SAME person as the attached
image: short messy dark navy-black hair, fair skin, red short-sleeve shirt,
dark charcoal shorts, brown leather boots. Flat cel-shaded color blocks, crisp
1px dark outline, no photorealism, no gradient, no blur, no depth of field.
Front-facing slight 3/4 view, full body, centered with a small margin, fully
transparent background (PNG alpha, no ground, no cast shadow). Identical camera,
identical scale, identical centering and lighting as the attached reference —
ONLY the legs and arms change for this walk-cycle frame. Clean, readable, game-
ready sprite frame.
```

### 8.3 四帧各自的姿势（把对应一句接在 Base prompt 后面）

| 文件 | 帧 | 接在 base 后面的英文 |
|---|---|---|
| `jpt-walk1.png` | 接触 A | `Walk cycle frame 1 of 4 (contact pose): LEFT leg striding forward with bent knee and heel landing ahead, RIGHT leg extended trailing behind, slight forward body lean; RIGHT arm swung forward, LEFT arm swung back. Matches the attached walking reference.` |
| `jpt-walk2.png` | 过渡 A | `Walk cycle frame 2 of 4 (passing pose): both legs close together passing directly under the torso, supporting leg straight, body at its lowest point; both arms near neutral close to the sides. Same character, same outfit, same framing.` |
| `jpt-walk3.png` | 接触 B | `Walk cycle frame 3 of 4 (contact pose, mirrored): RIGHT leg striding forward with bent knee and heel landing ahead, LEFT leg extended trailing behind, slight forward body lean; LEFT arm swung forward, RIGHT arm swung back.` |
| `jpt-walk4.png` | 过渡 B | `Walk cycle frame 4 of 4 (passing pose, lift): both legs close together passing under the torso, body at its highest point of the stride (slight upward bob); both arms near neutral close to the sides.` |

循环顺序：1 → 2 → 3 → 4 → 1…（每帧约 120ms，约 0.5s 一轮）。

### 8.4 翻车救法（沿用 §2.3 同思路）

| 现象 | 追加到 prompt 末尾 |
|---|---|
| 四帧人不像 / 衣服变色 | `Keep the EXACT same face, hairstyle, shirt red, shorts charcoal, boots brown as the attached image. Do not redesign the character.` |
| 角色大小/位置每帧乱跳 | `The character must occupy the exact same size and the exact same centered position in the frame as the attached reference. Do not zoom, crop, or recompose.` |
| 背景不透明 / 有地面阴影 | `Fully transparent background. No floor, no ground texture, no drop shadow, no vignette.` |
| 变成横版侧身走 | `Front-facing slight 3/4 view, NOT a side profile. The character faces the viewer while stepping.` |
| 出了照片感 / 糊 | `Flat hard-edged cel-shaded pixel-game art only. No photorealism, no soft shading, no blur.` |

### 8.5 验收（4 张都要过）

- [ ] 4 张和 `jpt-stand1/2` 一眼同一个人、同配色、同机位同缩放
- [ ] 透明背景干净（无地面/无投影/无白边）
- [ ] 1→2→3→4 连起来看是一个顺滑的走路循环（左右腿交替、有上下起伏）
- [ ] 帧间角色不跳动（位置/大小一致）
- [ ] 命名严格为 `jpt-walk1.png`…`jpt-walk4.png`，覆盖进 `assets/sprites/`
